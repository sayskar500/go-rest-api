...
import (
	...
	"github.com/sayskar500/go-rest-api/internal/storage"
	"github.com/sayskar500/go-rest-api/internal/types"
	"github.com/sayskar500/go-rest-api/internal/utils/response"
)
...
func New(storage storage.Storage) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := validator.New().Struct(studentInfo); err != nil {
			validateErrs := err.(validator.ValidationErrors)
			response.WriteJson(w, http.StatusBadRequest, response.ValidationError(validateErrs))
			return
		}

		lastId, err := storage.CreateStudent(studentInfo.Name, studentInfo.Email, studentInfo.Age)
